...
def between_session_pca(animal_folder_path, ref_session_id, ref_electrode='EEG8', n_components=3):
    '''
    ...
    ## Get animal id
    animal_id = os.path.basename(animal_folder_path)

    ## Read through Features + Save in Dictionary 
    features_dict = dict()

    first_file = True
    for feature_file_path in glob.glob(f'{animal_folder_path}/*/features/*{ref_electrode}*.csv.gz', recursive=True):

        ## Get session ID
        session_id = uts.parse_bids_session(feature_file_path)
        print(session_id) 
        print(os.path.basename(feature_file_path))
        print()
        # features_dict[session_id] = pd.read_csv(feature_file_path)

    ## SANITY CHECK: What to do when session recordings aren't of the same length 

    ## Extract PCA of ref electrode 
    ratio, ref_pca = extract_PCA(features_dict[session_id], n_components)
    ...
    pca_folder_path = os.path.join(session_folder_path, 'between_sessions_pca')
...
between_session_pca(animal_folder, '123', ref_electrode='EEG8', n_components=3)